...
export function ShapExplainer({ patientId, patient, latestVitals }: Props) {
  const [risk, setRisk] = useState<RiskScoreResponse | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    if (!latestVitals) { setLoading(false); return }
  ...
  return (
    <div className="bg-white rounded-xl border p-4">
      <h2 className="text-sm font-semibold text-gray-700 mb-3">ML Risk Explanation</h2>

      {loading && <div className="h-20 bg-gray-100 rounded animate-pulse" />}

      {!loading && !risk && (
        <p className="text-xs text-gray-400">ML service offline — start FastAPI on :8000</p>
      )}
      ...
      {risk && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full ${TIER_COLORS[risk.risk_tier as keyof typeof TIER_COLORS]}`} />
            <span className="font-bold capitalize text-sm">{risk.risk_tier} risk</span>
            <span className="text-xs text-gray-400 ml-auto">{(risk.probability * 100).toFixed(0)}% confidence</span>
          </div>

          {/* SHAP bars */}
          <div>
            <p className="text-xs text-gray-500 mb-2">Top contributing factors:</p>
            {risk.shap_features.slice(0, 3).map((f) => {
              const maxImpact = Math.max(...risk.shap_features.map((x) => Math.abs(x.impact)))
              ...
              return (
                <div key={f.feature} className="mb-2">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-gray-600">{FEATURE_LABELS[f.feature] ?? f.feature}</span>
